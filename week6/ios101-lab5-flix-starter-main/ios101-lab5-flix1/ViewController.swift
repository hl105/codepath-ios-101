...
class ViewController: UIViewController, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // get row where cell will be placed using 'row' property on the passed in 'indexPath' (i.e. 'indexPath.row')
        let movie = movies[indexPath.row]
        
        //configure cell (i.e. update UI elements like labels, image views, etc.)
    ...
    private var movies: [Movie] = [] // make properties private if only need to be accessed within particular class. & for any properties added in viewcontroller classes, typical to eitehr assign initial value or make property optional.
        ...
        let session = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async { [weak self] in
                    ...
                    self?.movies = movies // since we're accessing view controller's movies property within a closure, we need to explicitly reference self and since we captured self as [weak self] to avoid retain cycles, self will be an aoptional hence ?.
