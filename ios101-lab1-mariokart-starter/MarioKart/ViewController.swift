...
class ViewController: UIViewController,
  ...
  @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
    ...
    // YOUR CODE HERE
      translate(kart: sender.view, by: view.frame.width) {
          self.translate(kart: sender.view, by: -self.view.frame.width) // using trailing closure syntax
      }
  }
  ...
  private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
    // YOUR CODE HERE
      ...
      kartView?.center = location // move cart to location
  }
...
//      }
      ...
      getKartReadyToRace(kart: self.kartView0) {
          self.getKartReadyToRace(kart: self.kartView1){
              self.getKartReadyToRace(kart: self.kartView2){
                  completion?() //runStartAnimationsOnebyOne also takes in optimal completion closure, which we execute after the three animations are done
              }
          }
      }
  }
  ...
  private func raceKartsWithSameSpeed() {
    // YOUR CODE HERE
      translate(kart: self.kartView0, by: view.frame.width)
      translate(kart: self.kartView1, by: view.frame.width)
      translate(kart: self.kartView2, by: view.frame.width)
  }
  ...
  private func raceKartsWithRandomizedSpeed() {
    // YOUR CODE HERE
      let kartView0Speed = Double.random(in: 0.5...5) // get random value between 0.5 and 5
