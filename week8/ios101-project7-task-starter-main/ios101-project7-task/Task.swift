...
extension Task {
    ...
    static func save(_ tasks: [Task]) {
        ...
        let encodedData: Data = try! JSONEncoder().encode(tasks)
    ...
    static func getTasks() -> [Task] {
        ...
        let defaults: UserDefaults = UserDefaults.standard
        if let data: Data = defaults.data(forKey: tasksKey) {
            let decodedTasks: [Task] = try! JSONDecoder().decode([Task].self, from: data)
            return decodedTasks
        } else {
